...
public class Counter extends SensorBase implements CounterBase, LiveWindowSendable
{
     ...
     * Create an up-Counter instance given a channel. The default digital module
     * is assumed.
     *
     ...
     * associated
     * input channel. The counter value is not reset on starting, and still has
     * the previous value.
     */
    public void start()
    {
     ...
     * If you
     * enable the update when empty, you will be notified of the stopped source
     * and the event
     * time will report 0 samples. If you disable update when empty, the most
     * recent average
     * will remain on the output until a new sample is acquired. You will never
     * see 0 samples
     * output (except when there have been no events since an FPGA reset) and
     * you will likely not
     * see the stopped bit become true (since it is updated at the end of an
     ...
     * set using the
     * SetMaxPeriod method. If the clock exceeds the MaxPeriod, then the device
     * (and counter) are
